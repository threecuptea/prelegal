        ...
        {config.fields
          ...
          .map(({ key, label, format }) => {
            ...
            const display = raw != null && raw !== ''
              ? (format ? format(raw, data) : String(raw))
              : null
            return (
              <div key={key}>
                ...
                <dd className="text-gray-800">
                  {display ?? (
                    <span className="italic text-gray-300">[Not yet provided]</span>
                  )}
                </dd>
              </div>
            )
...
export function downloadMarkdown(data: DocumentFields, documentType: DocumentType): void {
  const config = DOCUMENT_REGISTRY[documentType]
  const markdown = generateCoverPage(data, documentType)
  const blob = new Blob([markdown], { type: 'text/markdown' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url
  a.download = config.displayName.replace(/\s+/g, '-') + '-cover.md'
  a.click()
